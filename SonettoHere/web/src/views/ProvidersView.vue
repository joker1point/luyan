      ...
      <div v-if="discoveredModels.length > 0" class="form-section">
        ...
        <div class="model-search-row">
          <input
            ...
            placeholder="🔍 输入关键字过滤模型（如 qwen、turbo、235b）"
          />
          <button v-if="modelSearch" type="button" class="btn sm" @click="modelSearch = ''">清除</button>
        </div>
        <div class="model-list">
          <label v-for="m in filteredModels" :key="m" class="model-item">
            <input type="checkbox" :value="m" :checked="selectedModels.includes(m)" @change="toggleModel(m)" />
            <span v-html="highlightKeyword(m)"></span>
          </label>
          <div v-if="filteredModels.length === 0" class="model-empty">没有匹配的模型</div>
        </div>
        <div class="model-bulk-actions">
          ...
          <span v-if="modelSearch" class="model-filter-hint">
            匹配 {{ filteredModels.length }} / {{ discoveredModels.length }}
          </span>
        </div>
      </div>
...
const filteredModels = computed(() => {
  ...
  if (!kw) return discoveredModels.value
  return discoveredModels.value.filter((m: string) => m.toLowerCase().includes(kw))
})

function highlightKeyword(model: string) {
  const kw = modelSearch.value.trim()
  if (!kw) return escapeHtml(model)
  const escapedModel = escapeHtml(model)
  const escapedKw = escapeHtml(kw)
  // 不区分大小写替换，保留原大小写
  const re = new RegExp(escapedKw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi')
  return escapedModel.replace(re, match => `<mark class="kw-hit">${match}</mark>`)
}
...
// 切换 provider/重新拉取时清空搜索框
watch(discoveredModels, () => {
  modelSearch.value = ''
})
...
.model-search-row .model-search {
  flex: 1;
  font-size: 13px;
}
.model-search-row .model-search:focus {
  border-color: var(--accent);
}
